...
export default {
  ...
  computed: {
    dataTable() {
      const listData = this.$store.state.profileModule.listSexual;
  ...
  methods: {
    ...
    async handelAPI() {
      ...
      if (formData._id) {
        await this.updateSexual(formData).then((data) => {
          this.notificationSuccess("Update record successfully");
          this.dialogFormVisible = false;
        });
      } else {
        await this.insertSexual(formData).then((data) => {
          this.notificationSuccess("Insert record successfully");
        });
      }
      this.$refs.dataFormRef.resetFields();
      // await this.updateSexual(this.dataFormAction).then((data) => {
    ...
    async onChangeDeleteData(val) {
      await this.deleteSexual(val).then((data) => {
        this.notificationSuccess();
      });
