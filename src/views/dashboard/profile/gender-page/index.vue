...
export default {
  ...
  methods: {
    ...
    async onConfirmDialog() {
      ...
      // await this.updateGender(this.dataFormAction).then((data) => {
    ...
    async handelAPI() {
      ...
      if (formData._id) {
        await this.updateGender(formData).then((data) => {
          this.notificationSuccess("Update record successfully");
          this.dialogFormVisible = false;
        });
      } else {
        await this.insertGender(formData).then((data) => {
    ...
    async onChangeDeleteData(val) {
      await this.deleteGender(val).then((data) => {
        this.notificationSuccess();
      });
